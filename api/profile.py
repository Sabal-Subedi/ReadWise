    ...
    try:
        ...
        if(cart):
            ...
            for cartItem in cart:
                ...
                book = store_data_uncleaned[store_data_uncleaned["isbn"]
                                            == cartItem.book_isbn]
                ...
                if(not book.empty):  # always check if the dataframe is empty like this
                    ...
                    print(cartdf)
            return cartdf.to_json(orient='index'), 200
        # if cart empty return null
        return jsonify(cart=None), 500  # none = null
    except Exception as e:
        print(e)
...
def removefromcart():
    try:
        bookisbn = request.get_json().get('isbn')
        existingCartItem = CartItem.query.filter_by(book_isbn=bookisbn).first()
        if(existingCartItem):
            db.session.remove(existingCartItem)
            return jsonify(bookisbn=bookisbn, msg="Deleted the cart item"), 200
