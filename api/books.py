...
store_data = store_data_uncleaned.drop(columns=["book_edition","book_review_count","book_format"])    
...
def getmostpopular():
    try: 
        count = None if request.args.get('count') is None else int(request.args.get('count')) 
        movie_sorted_ranking=store_data.sort_values("book_rating_count",ascending=False).head(100 if count is None else count)
...
def gettopfiction():
    try: 
        count = None if request.args.get('count') is None else int(request.args.get('count'))
        store_data_final = store_data[store_data['genre'].str.contains('fiction|fantasy|magic', case=False, regex=True)]
...
def gettopromance():
    try: 
        count = None if request.args.get('count') is None else int(request.args.get('count'))
        store_data_final = store_data[store_data['genre'].str.contains('love|romance|adult', case=False, regex=True)]
        return calculateWeightedRating(store_data_final,count)  
    except Exception as e:
        print(e)
        return jsonify(msg = "Error occured. Check out error message", errmsg = str(e)), 500 
...
def gettophorror():
    try: 
        count = None if request.args.get('count') is None else int(request.args.get('count'))
        store_data_final = store_data[store_data['genre'].str.contains('ghost|horror|paranormal|apocalyptic', case=False, regex=True)]
        return calculateWeightedRating(store_data_final, count)  
    except Exception as e:
        print(e)
        return jsonify(msg = "Error occured. Check out error message", errmsg = str(e)), 500 
...
def gettopmystery():
    try: 
        count = None if request.args.get('count') is None else int(request.args.get('count'))
        store_data_final = store_data[store_data['genre'].str.contains('mystery|puzzle|secret|enigma|riddle', case=False, regex=True)]
        return calculateWeightedRating(store_data_final, count)  
    except Exception as e:
        print(e)
        return jsonify(msg = "Error occured. Check out error message", errmsg = str(e)), 500 
...
def gettopthriller():
    try: 
        count = None if request.args.get('count') is None else int(request.args.get('count'))
        store_data_final = store_data[store_data['genre'].str.contains('thriller', case=False, regex=True)]
        return calculateWeightedRating(store_data_final, count)  
    except Exception as e:
        print(e)
        return jsonify(msg = "Error occured. Check out error message", errmsg = str(e)), 500 
...
def gettopscifi(): 
    try:    
        count = None if request.args.get('count') is None else int(request.args.get('count'))
        store_data_final = store_data[store_data['genre'].str.contains('science|Post Apocalyptic|Steampunk|Dystopia|time travel', case=False, regex=True)]
        return calculateWeightedRating(store_data_final, count)  
    except Exception as e:
        print(e)
        return jsonify(msg = "Error occured. Check out error message", errmsg = str(e)), 500 
